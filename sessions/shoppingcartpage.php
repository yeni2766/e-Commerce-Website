...
if (isset($_POST['productid'], $_POST['quantityofitem'] ,$_POST['product'], $_POST['product_price' ]) && is_numeric($_POST['productid']) && is_numeric($_POST['quantityofitem'])) {
   //is_numeric is used to make sure the data from the variables are numbers only
   
    $productid = (int)$_POST['productid'];
    $quantityofitem = (int)$_POST['quantityofitem'];
	$product1 = trim($_POST["product"]);
	$product2 = trim($_POST["product_price"]);
	 $_SESSION["product"] = $product1; 
	 $_SESSION["price"] = $product2; 
...
<div class="mycart">
 ...
 <div class="buttons">
            ...
            <input type="submit" class = "btn" value="Buy Now" name="buyproduct">
        </div>
		</form>
<?=template_footer()?>
