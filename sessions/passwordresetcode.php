...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["newpassword"]))){
        ...
        $newpassworderrormessage = "Please enter the new password to change your password before you can proceed <br>";
...
echo $confirmnewpassworderrormessage;
       ...
       //this is if the two error messages are not set which means there has not been any erros
    if(empty($newpassworderrormessage) && empty($confirmnewpassworderrormessage)){
        ...
        $sql = "UPDATE users SET userPWD = ? WHERE userName = ?";
        ...
        if($result = mysqli_prepare($conn, $sql)){
            ...
            // Set parameters
            $binded_newpassword = ($newpassword);
